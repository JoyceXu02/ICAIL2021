...
def trainer(bertmodel, cnnmodel, device, train_dataloader, val_dataloader, epochs, batch_size, lr, loss = 'crossentropy'):
	...
	for epoch in range(epochs):
		...
		for i, data in enumerate(train_dataloader):
			...
			with torch.no_grad():
				output_embed = bertmodel(ids, mask, token_type_ids)
		...
		if avg_val_loss < min_val_loss:
			...
			torch.save(model, f'saved_bcnn_models/tut-model.pt')
		print(f'Epoch: {epoch}')
...
if __name__ == "__main__":
	...
	preds = evalute(best_model, device, test_loader, loss=args.loss)
