...
def accuracy(preds, y):
	...
	return max_preds, torch.sum(correct)/len(correct)
...
# model, device, train_dataloader, valid_dataloader, epochs, batch_size, lr, loss = 'crossentropy'
...
def trainer(model,
	...
	for epoch in range(epochs):
		...
		if avg_val_loss < min_val_loss:
			...
			if not os.path.exists('saved_lstm_models'):
				os.mkdir('saved_lstm_models')
...
if __name__ == "__main__":
	...
	my_parser.add_argument("--embed_size", type=int,
							default=300)
	my_parser.add_argument("--hidden_dim", type=int,
							default=300)
	my_parser.add_argument("--n_layers", type=int,
							default=1)
	...
	# load model
	model = LSTM(vocab_size = vocab_size, embed_size= args.embed_size,
				hidden_dim = args.hidden_dim, output_dim=4, padding_idx = vocab['<pad>'],
